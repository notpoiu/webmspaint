  ...
  if (hash === signature) {
    ...
    return new Response(
      `Thank you for purchasing ${data.quantity} mspaint key(s)!\n` +
        `You can redeem your serial(s) at: https://www.mspaint.cc/purchase/completed?serial=${encodeURIComponent(
          createdSerials.join(",")
        )}\n\n` +
...
export async function GET(request: NextRequest) {
  ...
  if (isDev ? false : !request_ip) {
    return NextResponse.json({
      status: 400,
      error:
        "bad request (ip not found), please contact support (https://discord.gg/Q6gHakV36z)",
    });
  }
  ...
  if (!order_id || !order_email) {
    return NextResponse.json({
      status: 400,
      error:
        "bad request, please contact support (https://discord.gg/Q6gHakV36z)",
    });
  }
  ...
  const deliverablesResponse = await fetch(
    `https://sell.app/api/v2/invoices/${order_id}/deliverables`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!response.ok) {
    return NextResponse.json({
      status: 500,
      error:
        "internal server error, please contact support (https://discord.gg/Q6gHakV36z)",
    });
  }
  ...
  if (
    invoiceData.customer_information.email !== order_email ||
    invoiceData.status.status.status !== "COMPLETED" ||
    (!isDev && invoiceData.customer_information.ip !== request_ip)
  ) {
    return NextResponse.json({
      status: 400,
      error:
        "bad request (data mismatch), please contact support (https://discord.gg/Q6gHakV36z)",
    });
  }
  ...
  const { rows } =
    await sql`SELECT * FROM mspaint_keys WHERE order_id = ${order_id};`;
  ...
  if (rows.length > 0 && !isAllClaimed) {
    return redirect(
      "/purchase/completed?serial=" +
        encodeURIComponent(rows.map((row) => row.serial).join(","))
    );
  }
  ...
  return NextResponse.json({
    status: 400,
    error:
      "bad request (all keys claimed), please contact support (https://discord.gg/Q6gHakV36z)",
  });
}
