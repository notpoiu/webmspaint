...
export const ElementParser: React.FC<{ element: UIElement; stateKeyPrefix?: string; }> = ({ element, stateKeyPrefix }) => {
	...
	const core = (() => {
		switch (element.type) {
			...
			default:
				return <div className="text-red-400 text-left">
	...
	return (
		<TabContainer>
			<TabLeft>
				...
				{
					groupboxes?.Left && Object.values(groupboxes.Left).sort((a, b) => (a.order ?? 0) - (b.order ?? 0)).map((gb) => (
			...
			<TabRight>
				...
				{
					groupboxes?.Right && Object.values(groupboxes.Right).sort((a, b) => (a.order ?? 0) - (b.order ?? 0)).map((gb) => (
						<Groupbox key={gb.name} title={gb.name}>
							{gb.elements.map((el) => (
								<ElementParser
