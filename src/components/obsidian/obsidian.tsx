...
/**
 * Render an Obsidian-style UI container with configurable header, searchable tab list, content panels, and footer.
 *
 * If `uiData` is not provided the component renders a brief loading placeholder. When `uiData` is present, tabs are
 * generated from `uiData.tabs`, sorted by each tab's `order` value, and their content is rendered via `TabParser`.
 * Tab icons (when specified) are resolved dynamically from the `lucide-react` icon set.
 *
 * @returns A React element containing the complete Obsidian UI (or a loading paragraph if `uiData` is undefined).
 */
