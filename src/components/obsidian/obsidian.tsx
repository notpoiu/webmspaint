...
export function Obsidian({
    ...
    return (
        <div className={cn("w-[720px] h-[600px] rounded-[3px] bg-[rgb(15, 15, 15)] border-[rgb(40,40,40)] border relative font-normal", mono.className)}>
            ...
            {/* Tabs */}
            <Tabs defaultValue={"Home"} className="h-[calc(100%-69px)] w-full flex flex-row bg-[#111111]">
                <TabsList className="h-full border-r-[rgb(40,40,40)] border-r min-w-[calc(30%+1.2px)] flex flex-col justify-start bg-[rgba(15,15,15,255)] rounded-none p-0">
                    {Object.entries(uiData.tabs).sort(([, a], [, b]) => a.order - b.order).map(([tabName, tab], index) => {
                ...
                {Object.entries(uiData.tabs).map(([tabName, tab], index) => (
                    <TabsContent value={tabName} key={index} className="w-full max-w-[calc(100%-30%+1.2px)] bg-[#111111] p-0 mt-0">
